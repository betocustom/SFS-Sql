USE [leaseoper]
GO
/****** Object:  StoredProcedure [dbo].[pa_lo_interfaz_m31_acce01li_099001000]    Script Date: 06/03/2014 16:20:09 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

/****** Objeto:  procedimiento almacenado dbo.pa_lo_interfaz_m31_acce01li_099001000    fecha de la secuencia de comandos: 09-07-2007 9:25:28 ******/
ALTER PROCEDURE [dbo].[pa_lo_interfaz_m31_acce01li_099001000]
@fecha_proceso    SMALLDATETIME,
@salida           INT OUTPUT
AS
/*
Nombre            :  pa_lo_interfaz_m31_acce01li_099001000
...
EXEC leaseoper..pa_lo_interfaz_m31_acce01li_099001000 '19/03/2014',0
EXEC leaseoper..pa_lo_interfaz_m31_acce01li_099001000 '24/03/2010',0
...
@marca, @modelo, @tipo_vehiculo, @ano,@color,@placa_letras,@num_motor,@num_chasis
...
BEGIN
	...
	SET @fec_ter	= '00000000'	
	
	--campo sin uso
	SET @derec_veh	= ' '	
	
	--Cobertura de Seguro
	SET @cober_veh = '08' --definición banco de chile, para accesorios = 8
...
SELECT 
	...
	REPLICATE('0', 40 - LEN(rsoci_veh)) + rsoci_veh +
	REPLICATE('0', 8 - LEN(LTRIM(RTRIM(rutri_veh)))) + LTRIM(RTRIM(rutri_veh)) +
	REPLICATE('0', 1 - LEN(dveri_veh)) + dveri_veh +
	REPLICATE('0', 50 - LEN(LTRIM(RTRIM(miden_veh)))) + LTRIM(RTRIM(miden_veh)) +
	REPLICATE('0', 4 - LEN(LTRIM(RTRIM(crubr_veh)))) + LTRIM(RTRIM(crubr_veh)) +
	REPLICATE('0', 3 - LEN(LTRIM(RTRIM(nunid_veh)))) + LTRIM(RTRIM(nunid_veh)) +
	CONVERT(CHAR(9),vtasa_veh) +
	REPLICATE('0', 4 - LEN(LTRIM(RTRIM(vdedu_veh)))) + LTRIM(RTRIM(vdedu_veh)) +
	REPLICATE('0', 1 - LEN(filler_01)) + filler_01 +
	...
	REPLICATE('0', 25 - LEN(LTRIM(RTRIM(marca_veh)))) + LTRIM(RTRIM(marca_veh)) +
	REPLICATE('0', 25 - LEN(LTRIM(RTRIM(model_veh)))) + LTRIM(RTRIM(model_veh)) +
	...
	REPLICATE('0', 25 - LEN(LTRIM(RTRIM(color_veh)))) + LTRIM(RTRIM(color_veh)) +
	REPLICATE('0', 12 - LEN(LTRIM(RTRIM(npise_veh)))) + LTRIM(RTRIM(npise_veh)) +
	REPLICATE('0', 25 - LEN(LTRIM(RTRIM(nmarh_veh)))) + LTRIM(RTRIM(nmarh_veh)) +
	REPLICATE('0', 25 - LEN(LTRIM(RTRIM(chass_veh)))) + LTRIM(RTRIM(chass_veh)) +
	REPLICATE('0', 1 - LEN(derec_veh)) + derec_veh +
	REPLICATE('0', 2 - LEN(LTRIM(RTRIM(cober_veh)))) + LTRIM(RTRIM(cober_veh)) +
	REPLICATE('0', 2 - LEN(LTRIM(RTRIM(blanc_veh)))) + LTRIM(RTRIM(blanc_veh)) AS reg
INTO #final
FROM #vehiculos
...
GRANT EXEC ON pa_lo_interfaz_m31_acce01li_099001000 TO Usuarios
