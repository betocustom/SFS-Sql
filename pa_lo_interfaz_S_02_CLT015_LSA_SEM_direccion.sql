...
IF OBJECT_ID ('dbo.pa_lo_interfaz_S_02_CLT015_LSA_SEM_direccion') IS NOT NULL
	DROP PROCEDURE dbo.pa_lo_interfaz_S_02_CLT015_LSA_SEM_direccion
GO

CREATE PROCEDURE [dbo].[pa_lo_interfaz_S_02_CLT015_LSA_SEM_direccion]
    @fecha_proceso  SMALLDATETIME ,
    @salida	        TINYINT OUT
AS
/*
Nombre             : pa_lo_interfaz_S_02_CLT015_LSA_SEM_direccion
...
Autor              : Miguel Cornejo J
   ...
   EXEC pa_lo_interfaz_S_02_CLT015_LSA_SEM_direccion '16/12/2014',0
...
SELECT  SUBSTRING(REPLICATE('0',9 -DATALENGTH(RTRIM(CONVERT(CHAR(9),a.rut))))+ RTRIM(CONVERT(CHAR(9),a.rut)),1,9)+ --a.rut
     ...
     isnull((SELECT loc.descripcion 
         ...
         WHERE cdir.rut = a.rut
           AND cdir.cod_comuna = loc.cod_comuna_cia  ),REPLICATE('0',30))+--comun_agri	
       REPLICATE('0',5)+--filler_01
       REPLICATE(' ',129)--filler_01                  
...
GRANT EXECUTE ON dbo.pa_lo_interfaz_S_02_CLT015_LSA_SEM_direccion TO Usuarios
GO
