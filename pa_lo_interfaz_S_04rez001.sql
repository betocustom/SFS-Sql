...
IF OBJECT_ID ('dbo.pa_lo_interfaz_S_04rez001') IS NOT NULL
	DROP PROCEDURE dbo.pa_lo_interfaz_S_04rez001
GO

CREATE PROCEDURE [dbo].[pa_lo_interfaz_S_04rez001]
    @fecha_proceso  SMALLDATETIME ,
    @salida	        TINYINT OUT
AS
/*
Nombre             : pa_lo_interfaz_S_04rez001
...
Autor              : Miguel Cornejo J
   ...
   EXEC pa_lo_interfaz_S_04rez001 '16/12/2014',0
...
---Saldo insoluto
 ...
 UPDATE #universo
   SET  saldo_insoluto = (saldo_insoluto - ISNULL(abonos.capital_abono,0)) * 10000,
        tasa_ptmo      = POWER( (((tasa_spread+tasa_spread)/100.00)+1.00), (1.00 / (convert(FLOAT,cod_tipo_periodo_arr)/12))) - 1.00       
...
SELECT ' '+
      ...
      convert(CHAR(8),@fecha_proceso,112)+--fecha_proc        
	   ...
	   convert(CHAR(17),(b.provision_material+ b.provision_gasto_legal+b.provision_seguros)-b.monto_pie)+ --monto
	   convert(CHAR(8),b.fecha_ingreso_cont,112)+ --fec_curse
	   CASE b.cod_moneda_contrato WHEN 1 THEN  '1' 
        ...
        END+--tc_origen
	   convert(CHAR(8),b.fecha_termino,112)+--fec_termino
	   (SELECT str(sum((cuo.capital)),17,4) 
		  FROM t_cuotas cuo
		 WHERE cuo.operacion = b.operacion
		   AND cuo.estado = 1 )+ --saldo_cap 
	   convert(CHAR(17),'') + --saldo_insoluto
	          (SELECT str(count(*),17,4) 
		  FROM t_cuotas cuo
		 WHERE cuo.operacion = b.operacion
		   AND cuo.estado IN (1,3) )+ --num_cuotas
             --"Cartera Vigente Estado Credito --> 
      CASE WHEN exists (select 1 from leaseoper..t_contratos
       ...
       CASE WHEN exists (select 1 from leaseoper..t_contratos
                   where operacion not in (select operacion from t_contratos_castigados)
                     ...
                     THEN (SELECT convert(CHAR(08),min(cuo.fecha_vencimiento)) 
								   FROM t_cuotas cuo
								WHERE cuo.operacion = b.operacion
								  AND cuo.estado = 1
       ...
       CASE WHEN EXISTS (SELECT 1 operacion_nueva 
        ...
        ELSE 'N'
       END+ --renegociado
        (SELECT convert(CHAR(8),min(cuo.fecha_vencimiento)) 
	     FROM t_cuotas cuo
		WHERE cuo.operacion = b.operacion
		  AND cuo.estado = 1
       ...
       ((b.tasa_spread+b.tasa_variable)/12)+  --tasa_ptmo
       REPLICATE('0',17)+ --rebaja_cuota              
       SUBSTRING(REPLICATE('0',17 -DATALENGTH(RTRIM(str(e.cuota_min,13,4))))+ RTRIM(str(e.cuota_min,13,4)),1,17)+ --valor_prox_cuota            
       'M'+ --frec_pago
		...
		CASE (select datepart(dw,@fecha_proceso))
		 ...
		 ELSE ''
		END+  --solicitud 
		@fecha_proceso+ --fecha_infor       
        REPLICATE('0',5)+ --convecartera
        'LSA'+ --sistema      
        (SELECT cuo.fecha_vencimiento 
           FROM t_cuotas cuo 
        ...
        e.num_cuota_min+ --numero_cuota
        CASE b.cod_moneda_contrato WHEN 1 THEN  convert(CHAR(9),(10000)) 
         ELSE (SELECT  convert(CHAR(9),(p.valor)*10000)
				 FROM t_cuotas cuo,
...
GRANT EXECUTE ON dbo.pa_lo_interfaz_S_04rez001 TO Usuarios
GO
