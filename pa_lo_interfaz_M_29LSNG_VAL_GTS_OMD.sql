...
SELECT '0'+
       ...
       a.dv+       
       convert(CHAR(12),c.cod_material)+ --mater
       convert(CHAR(12),c.cod_rubro)+    --cbien,
       str(c.valor_libro_inicial_mat*100,12,3)+ --vcome
       str((c.valor_libro_inicial_mat*0.8)*100,12,3)+ --vgaran    
       isnull(convert(CHAR(8),c.fecha_tasacion,112), REPLICATE('0',8))+--ftasa
       '999'+--filler_01
       ...
       '00'+--filler4
      convert(CHAR(9),b.cod_oficina_real)+--llave1,
	    isnull((SELECT convert(CHAR(9),id_cliente) 
	     FROM t_datos_cliente_banco dc
	    WHERE dc.lberut = b.rut_cliente ),REPLICATE('0',2))+ --llave2 /* es un campo nuevo a agregar, aun no se realiza el alter*/	   
