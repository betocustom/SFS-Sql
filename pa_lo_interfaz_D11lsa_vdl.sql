...
IF OBJECT_ID ('dbo.pa_lo_interfaz_D11lsa_vdl') IS NOT NULL
	DROP PROCEDURE dbo.pa_lo_interfaz_D11lsa_vdl
GO

CREATE PROCEDURE [dbo].[pa_lo_interfaz_D11lsa_vdl]
    @fecha_proceso  SMALLDATETIME ,
    @salida	    TINYINT OUT
AS
/*
Nombre             : pa_lo_interfaz_D11lsa_vdl
...
Autor              : Miguel Cornejo J
   ...
   EXEC pa_lo_interfaz_D11lsa_vdl '16/12/2014',0
...
SELECT '0'+
       ...
       '2'+ --tipo_reg
       SUBSTRING(REPLICATE('0',6 -DATALENGTH(RTRIM(CONVERT(CHAR(6),@num_reg))))+ RTRIM(CONVERT(CHAR(6),@num_reg)),1,6)+ --num_lineas
       REPLICATE('0',6)+
       REPLICATE('0',6)+
...
GO
GRANT EXECUTE ON dbo.pa_lo_interfaz_D11lsa_vdl TO Usuarios
GO
